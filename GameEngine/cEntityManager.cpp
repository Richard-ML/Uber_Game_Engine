...
int cEntityManager::loadGameFromXML(std::string filename) {
  ...
  /* Load the Area Info */
  root_node = theDoc.first_node("AreaInfos");
  for (rapidxml::xml_node<> *cArea_node = root_node->first_node("AreaInfo");
       cArea_node; cArea_node = cArea_node->next_sibling()) {

    g_pAreaInfo->loadFromXML(cArea_node);
  }
  /* Load the Textures */
  ...
  /* Load the TileMeshes */
  root_node = theDoc.first_node("TileMeshes");
  // iterate over each game asset and store its data in a cTileMeshEntry object
  for (rapidxml::xml_node<> *cTileMeshEntry_node =
           root_node->first_node("TileMesh");
       cTileMeshEntry_node;
       cTileMeshEntry_node = cTileMeshEntry_node->next_sibling()) {
    // load the TileMesh buffers
    bool success = false;
    std::string meshName =
        cTileMeshEntry_node->first_attribute("name")->value();
    // success =
    // g_pMeshManager->loadMeshTileFileIntoTileMap(cTileMeshEntry_node->first_attribute("path")->value(),
    // std::stof(cTileMeshEntry_node->first_attribute("scale")->value()),
    // std::stof(cTileMeshEntry_node->first_attribute("tileR")->value()),
    // std::stof(cTileMeshEntry_node->first_attribute("tileG")->value()),
    // std::stof(cTileMeshEntry_node->first_attribute("tileB")->value()));
    success = g_pMeshManager->loadMeshFileIntoGLBuffer(
        cTileMeshEntry_node->first_attribute("path")->value(),
        g_pMeshManager->m_MapMeshNameTocMeshEntry[meshName],
        std::stof(cTileMeshEntry_node->first_attribute("scale")->value()),
        g_pXML_Utils->attributeToBool(
            cTileMeshEntry_node->first_attribute("isStationary")));
    g_map_RGBToMesh[std::make_tuple(
        std::stof(cTileMeshEntry_node->first_attribute("tileR")->value()),
        std::stof(cTileMeshEntry_node->first_attribute("tileG")->value()),
        std::stof(cTileMeshEntry_node->first_attribute("tileB")->value()))] =
        &g_pMeshManager->m_MapMeshNameTocMeshEntry[meshName];
    if (!success) {
      std::cout << "We couldn't load a TileMesh. Please fix the XML File.."
                << std::endl;
      system("pause");
      return -1;
    }
  ...
  /* Load the World tile info */
  root_node = theDoc.first_node("Worlds");
  // Iterate over each world node
  for (rapidxml::xml_node<> *cWorldEntry_node = root_node->first_node("World");
       cWorldEntry_node; cWorldEntry_node = cWorldEntry_node->next_sibling()) {
    g_pTextureManager->loadWorldTilesFromImage(cWorldEntry_node);
  }

  /* Load the Sounds */
  root_node = theDoc.first_node("Sounds");
  // iterate over each game asset and store its data in a cMeshEntry object
  for (rapidxml::xml_node<> *cSound_node = root_node->first_node("Sound");
       cSound_node; cSound_node = cSound_node->next_sibling()) {
    // load the sound file
    bool success = false;
    success = g_pSoundManager->loadSound(cSound_node);

    if (!success) {
      std::cout << "We couldn't load a sound. Please fix the XML File.."
                << std::endl;
      system("pause");
      return -1;
    }
...
void cEntityManager::createSpiderAtPos(float x, float y, float z) {
  // cEntityManager_Impl::vec_Entities.push_back(cEntityManager_Impl::vec_Entities.at(2));
  // glm::mat4 tempMatrix =
  // cEntityManager_Impl::vec_Entities.at(cEntityManager_Impl::vec_Entities.size()
  // - 1)->vec_m_components.at(0)->m_Transform;
  // tempMatrix[3] += glm::vec4(x, y, z, 1.0f);
  // cEntityManager_Impl::vec_Entities.at(cEntityManager_Impl::vec_Entities.size()
